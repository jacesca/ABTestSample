"""
Sample A/B Test when distributions are not normal.
"""
...
for col in new_metrics.keys():
    ...
    msg += f'\nShapiro Test (Stat: {stat_control:.3f}, p-value: {p_control:.3f}): {msg_temp}'  # noqa
    ...
    msg += f'\nShapiro Test (Stat: {stat_control:.3f}, p-value: {p_test:.3f}): {msg_temp}'  # noqa

    # Swarm Plots
    plt.subplot(2, 2, 3)
    sns.swarmplot(data=df_combined, x='group', y=col, hue='group', palette=colors_list)  # noqa
    plt.xlabel('')
    plt.ylabel(col)
    plt.title(f'Comparison of {col}', fontsize=8)
    plt.tight_layout()
    ...
    plt.subplot(2, 2, 4)
    sns.swarmplot(data=df_combined, x='group', y=col, hue='group', palette=colors_list)  # noqa
    sns.violinplot(data=df_combined, x='group', y=col, color="r", alpha=0.8)  # noqa
    plt.xlabel('')
    plt.ylabel(col)
    plt.title(f'Comparison of {col}', fontsize=8)
    plt.tight_layout()
    ...
    # U-Test
    # The U-test, also known as the Mann-Whitney test, is a nonparametric
    # statistical test used for comparing two independent samples. It is
    # used when the data do not meet the assumptions of parametric tests,
    # such as when the data distribution is non-normal or when the data
    # are measured on an ordinal scale.
    # It does not require levene tests.
    # The U-test assesses whether there is a statistically significant
    # difference between the medians of the two groups. It does not rely
    # on any prior assumptions about the data distribution and is relatively
    # robust to outliers or unusual values.
    # H₀: The medians of the 'Conversion' metric in the control and test
    #     groups are equal.
    # Hₐ: The medians of the 'Conversion' metric differ between the control
    #     and test groups.
    statistic, p_value = mannwhitneyu(control_group[col], test_group[col])
    if p_value > 0.05:
        msg_temp = f'The medians are NOT statistically different.'  # noqa
    else:
        # In this case, the p-value is higher than the acceptable significance
        # level (0.05), indicating that there is insufficient evidence to
        # suggest that the mean values differ between the groups.
        msg_temp = f'The medians are statistically different.'  # noqa
    msg += f'\nU-Test (Stat: {statistic:.3f}, p-value: {p_value:.3f}): {msg_temp}'  # noqa

    plt.suptitle(msg, x=0.05, horizontalalignment='left', fontsize=10)
    plt.subplots_adjust(top=0.75, hspace=.5)
